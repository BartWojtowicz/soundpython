...
def assert_audios_equal(audio1, audio2):
    ...
    assert abs(audio1.metadata.duration_seconds - audio2.metadata.duration_seconds) < 0.1, "Durations should match within 0.1s"
...
def test_concat_mono():
    ...
    assert abs(result.metadata.duration_seconds - 
              (audio1.metadata.duration_seconds + audio2.metadata.duration_seconds)) < 0.1
